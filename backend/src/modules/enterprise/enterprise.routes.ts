...
builderEnterpriseRouter.get("/workspace", (request, response) => {
  response.json({ data: enterpriseService.workspace(actor(request)) });
});

builderEnterpriseRouter.patch("/workspace", (request, response) => {
...
builderEnterpriseRouter.get("/team", (request, response) => {
  response.json({ data: enterpriseService.team(actor(request)) });
});

builderEnterpriseRouter.post("/team/invite", (request, response) => {
...
builderEnterpriseRouter.patch("/team/:memberId", (request, response) => {
...
builderEnterpriseRouter.delete("/team/:memberId", (request, response) => {
  const member = enterpriseService.deleteMember(actor(request), String(request.params.memberId));
  if (!member) response.status(404).json({ error: "Workspace member not found" });
  else response.json({ data: member });
});

builderEnterpriseRouter.get("/security/audit-logs", (request, response) => {
  response.json({ data: enterpriseService.auditLogs(actor(request)) });
});

builderEnterpriseRouter.get("/usage/reports", (request, response) => {
  response.json({ data: enterpriseService.usageReports(actor(request)) });
});

builderEnterpriseRouter.post("/data/export", (request, response) => {
...
builderEnterpriseRouter.post("/data/delete-request", (request, response) => {
...
agentEnterpriseAdminRouter.get("/security/report", (request, response) => response.json({ data: enterpriseService.securityReport(request.session!.organizationId!) }));
agentEnterpriseAdminRouter.get("/reliability/report", (request, response) => response.json({ data: enterpriseService.reliabilityReport(request.session!.organizationId!) }));
agentEnterpriseAdminRouter.get("/compliance/report", (request, response) => response.json({ data: enterpriseService.complianceReport(request.session!.organizationId!) }));
agentEnterpriseAdminRouter.get("/launch-readiness", (request, response) => response.json({ data: enterpriseService.launchReadiness(request.session!.organizationId!) }));
