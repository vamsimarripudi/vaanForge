...
| Frontend follow-up rules | 840-877 | Active frontend code stays under `frontend/` with Next.js, TypeScript, responsive theme support, and required `src` folders. PDF-required feature folders expose `components`, `pages`, `hooks`, `services`, and `types`; screen states are already guarded by `scripts/qa-state-contract.js`. `scripts/qa-frontend-structure-contract.js` locks the folder side. |
